...
select s.OrderID,format(s.OrderDate,'dd.MM.yyyy') as Дата, month(s.OrderDate) as Месяц,
		...
		when  month(s.OrderDate) between '9' and '12' then '3'
		end) as Треть_года,
...
and t.PickingCompletedWhen is not null
...
SET DATEFORMAT mdy;
select 
c.DeliveryMethodName as [способ доставки],b.ExpectedDeliveryDate as [дата доставки],
a.SupplierName as [имя поставщика],d.FullName as [имя контактного лица]
from 
Purchasing.Suppliers a, Purchasing.PurchaseOrders b, Application.DeliveryMethods c, Application.People d
where
c.DeliveryMethodID=b.DeliveryMethodID and
b.ContactPersonID=d.PersonID and
a.SupplierID=b.SupplierID and
DeliveryMethodName in ('Air Freight','Refrigerated Air Freight') 
and ExpectedDeliveryDate between '01.01.2014' and '01.01.2014'
...
select top(10) a.InvoiceDate as [Дата продажи], b.FullName as [Имя клиента],c.FullName as [Имя сотрудника]
from 
Sales.Invoices a inner join Application.People b  on AccountsPersonID=PersonID inner join Application.People c on SalespersonPersonID=c.PersonID
...
from 
Application.People a join Sales.Invoices b on b.AccountsPersonID=a.PersonID
					 join Warehouse.StockItemTransactions c  on b.InvoiceID=c.InvoiceID
					 join Warehouse.StockItems d on d.StockItemID=c.StockItemID and d.StockItemName in ('Chocolate frogs 250g')
Order by a.PersonID;
...
select   year(b.InvoiceDate) as [Год продажи],month(b.InvoiceDate) as [Месяц продажи],avg(a.UnitPrice)as[Средняя цена],sum(a.UnitPrice) as [Общая сумма]
from Sales.OrderLines a join Sales.Invoices b on a.OrderID=b.OrderID
...
--Продажи смотреть в таблице Sales.Invoices и связанных таблицах.
select   year(b.InvoiceDate) as [Год продажи],month(b.InvoiceDate) as [Месяц продажи],sum(a.UnitPrice) as [Общая сумма продаж]
from Sales.OrderLines a join Sales.Invoices b on a.OrderID=b.OrderID
Group by year(b.InvoiceDate),month(b.InvoiceDate)
